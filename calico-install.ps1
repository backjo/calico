...
Param(
    [parameter(Mandatory = $false)] $ReleaseBaseURL="https://github.com/projectcalico/calico/releases/download/v3.17.1/",
    [parameter(Mandatory = $false)] $ReleaseFile="calico-windows-v3.17.1.zip",
...
if ($platform -EQ "eks") {
    $awsNodeName = Invoke-RestMethod -uri http://169.254.169.254/latest/meta-data/local-hostname -ErrorAction Ignore
    $awsNodeName = -join($awsNodeName, ".us-west-2.compute.internal")
